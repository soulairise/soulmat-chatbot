...
export const embedQuery = async (q: string): Promise<number[]> => (await embed(q))[0]
...
export async function streamChat(
  ...
  const r = await fetch(endpoint('chat'), {
    ...
    body: JSON.stringify(
      useDirect()
        ? { model: CHAT_MODEL, messages, temperature: 0, stream: true }
        : { messages },
    ),
  })
...
export async function chatJson<T>(
  ...
  const r = await fetch(endpoint('chat'), {
    ...
    body: JSON.stringify(
      useDirect()
        ? { model: CHAT_MODEL, messages, temperature: 0, response_format }
