...
export function buildSystemPrompt(): string {
  return [
    ...
    '8. 제품별 안내에는 자료에 적힌 정확한 제품명을 함께 씁니다. 트래블매트의 세탁법을 다른 매트나 모든 요가매트의 세탁법으로 넓히지 않습니다.',
    '9. 질문에 제품명이 없으면 먼저 자료로 확인한 제품명을 밝혀 그 제품에 한해서 안내하고, 다른 제품은 제품명을 물어봅니다. 사용자가 특정한 제품의 자료가 없으면 다른 제품의 안내로 대신 답하지 않습니다.',
    '10. 한국어로, 3~5문장 이내로 간결하게 답합니다.',
  ].join('\n')
}
...
export function buildUserPrompt(question: string, r: Retrieval): string {
  ...
  if (r.weakEvidence) {
    parts.push(
      '',
      `[주의] 위 자료와 질문의 최고 유사도가 ${r.maxCosine.toFixed(3)}으로 낮습니다(기준 ${WEAK_EVIDENCE_THRESHOLD}).`,
